  ...
  public sealed class ConfigurationBuildersSection : ConfigurationSection
  {
    ...
    public ConfigurationBuilder GetBuilderFromName(string builderName)
    {
      ...
      if (strArray.Length == 1)
      {
        ...
        if (builder == null)
          throw new Exception(SRPorted.GetString("Config_builder_not_found", new object[1]
          {
            (object) builderName
          }));
        try
      ...
      foreach (string str in strArray)
      {
        ...
        try
        {
          configurationBuilderChain.Builders.Add(InstantiateBuilder(builder));
        }
        catch (FileNotFoundException)
        {
          if (flag)
    ...
    private ConfigurationBuilder InstantiateBuilder(ProviderSettings ps)
    {
      ...
      if (!typeof (ConfigurationBuilder).IsAssignableFrom(reflectionPermission))
        throw new Exception(SRPorted.GetString("WrongType_of_config_builder"));
      if (!TypeUtil.IsTypeAllowedInConfig(reflectionPermission))
        throw new Exception(SRPorted.GetString("Type_from_untrusted_assembly", new object[1]
        {
          (object) reflectionPermission.FullName
        }));
      
      return CreateAndInitializeBuilderWithAssert(reflectionPermission, ps);
    }
